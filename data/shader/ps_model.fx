...
PS_OUTPUT main(PS_INPUT_MODEL input)
{
    ...
    float shadow = 1.0f; // シャドウの初期値 (1.0:影なし, 0.0:影あり)
	{
        ...
        for (int i = 0; i < SAMPLE_NUM; ++i)
        {
            ...
            shadow += ShadowmapTexture.SampleCmp(ShadowmapSampler, shadowUv + offset * float2(0.25f, 1.0f), screenPosition.z - shadowBias[cascadeIndex]).r;
        }
   ...
   //output.color0_.rgb = lerp(output.color0_.rgb, float3(0.4, 0.6, 1), ratio);
    ...
    //float3 specularColor = lerp(specularMapColor.rgb, albedo, length(specularMapColor));
    ...
    output.color1_ = float4(roughness, metallic, 0, 1);
    output.color2_ = float4(N, 1); // 法線
    output.color3_ = float4(input.world_position_, 1);
