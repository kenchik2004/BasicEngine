...
namespace NeonFade {
	...
	void PlayerAttack1State::OnEnter(IStateMachine* machine)
	{
		...
		rb->velocity = owner_player->transform->AxisZ() * 30 + Vector3(0, 20, 0);
	...
	void PlayerAttack1State::Update(IStateMachine* machine, float dt)
	{
		...
		if (exit_timer <= SPIN_TIME) {
			...
			animator->anim_speed = exit_timer * smoothstep_rottime * -2.0f;
			...
			animator->anim_speed = 2.5f;
			if (!hit_box) {
				auto col = owner_player->AddComponent<SphereCollider>
					(Vector3(0, 3, 5), Quaternion(0, 0, 0, 1), 5.0f,
