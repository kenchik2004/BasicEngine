...
namespace Sample
{
	int SampleAnimationObject::Init()
	{
		...
		std::function on_jump_finish = [=]() {my_animator->Play("run", true, 0.0f, 0.4f); };
		...
		my_animator->SetAnimationCallBack("jump", on_jump_finish, 60, "on_jump_finish");
	...
	void SampleAnimationObject::Update()
	{
		...
		if (Input::GetKeyDown(KeyCode::Alpha3))
			my_animator->PlayIfNoSame("run", true,0.0f,0.4f);
