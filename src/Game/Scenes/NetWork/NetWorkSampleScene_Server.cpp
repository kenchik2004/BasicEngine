...
namespace NetWorkTest_Server {
	...
	void NetWorkSampleScene_Server::ProcessReceivedUDPBytes(u8* data, size_t length) {
	...
	void NetWorkSampleScene_Server::ProcessReceivedBytes(u8* data, size_t length) {
		if (!data || length == 0) return;
			...
			case PACKET_TYPE_TEXT: {
				...
				if (txt == received_text.end()) {
					...
					txt->second.assign(reinterpret_cast<char*>(payload),
						reinterpret_cast<char*>(payload) + hdr->sizeBytes);
				for (int i = 0; i < chat_network.size(); i++) {
					auto net = chat_network[i];
					if (hdr->ip != net->ip)
						SendPacket(net_manager.get(), net, PACKET_TYPE_TEXT, payload,
