...
namespace NetWorkTest_Client {
	...
	static constexpr unsigned short open_port = 11452;
	static constexpr unsigned short open_udp_port = 11453;

	IPDATA another_ip = { 127,0,0,0 };
	unsigned short another_port = 11452;
	unsigned short another_udp_port = 11453;
	...
	static void SendPacket(NetWorkManagerBase* net_manager, NetWork* net, PacketType type, const void* payload, u32 sizeBytes, IPDATA overrided_ip = { 0,0,0,0 }) {
	...
	static void SendPacket(NetWorkManagerBase* net_manager, UDPNetWork* net, IPDATA ip, unsigned short port, PacketType type, const void* payload, u32 sizeBytes, IPDATA overrided_ip = { 0,0,0,0 }) {
		if (!net_manager) return;
		if (!net) return;
		const auto buf = net_manager->CreatePacket(type, payload, sizeBytes, overrided_ip);
	...
	void NetWorkSampleScene_Client::ProcessReceivedUDPBytes(u8* data, size_t length) {
	...
	void NetWorkSampleScene_Client::ProcessReceivedBytes(u8* data, size_t length) {
		if (!data || length == 0) return;
	...
	int NetWorkSampleScene_Client::Init()
	{
		...
		net_manager->SetOnDisconnectionCallback([this](NetWork* net) {
			is_connected = false;
			auto it = std::find(chat_network.begin(), chat_network.end(), net);
	...
	void NetWorkSampleScene_Client::Update()
	{
		...
		if (is_connected) {
			...
			if (Input::GetKeyDown(KeyCode::Return)) {
				...
				std::string segment;
				std::istringstream tokenStream(input_text);
	...
	void NetWorkSampleScene_Client::LateDraw()
	{
		...
		for (int i = 0; i < chat_network.size() && i < received_text.size(); i++) {
		...
		DxLib::DrawBox(150 + input_text.size() * 12, 421, 160 + input_text.size() * 12, 430, 0xffffff, true);
