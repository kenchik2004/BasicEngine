...
{
	DxLib::GetGraphSize(dxlib_handle, reinterpret_cast<int*>(&width), reinterpret_cast<int*>(&height));

	auto* d3d_device = reinterpret_cast<ID3D11Device*>(const_cast<void*>(GetUseDirect3D11Device()));
	ID3D11Resource* d3d_resource = reinterpret_cast<ID3D11Resource*>(const_cast<void*>(GetGraphID3D11Texture2D(dxlib_handle)));
	texture = d3d_resource;
	D3D11_TEXTURE2D_DESC desc;
	reinterpret_cast<ID3D11Texture2D*>(d3d_resource)->GetDesc(&desc);
	if (desc.BindFlags & D3D11_BIND_RENDER_TARGET)
		rtv = reinterpret_cast<ID3D11RenderTargetView*>(const_cast<void*>(GetGraphID3D11RenderTargetView(dxlib_handle)));
	if (desc.BindFlags & D3D11_BIND_DEPTH_STENCIL)
		dsv = reinterpret_cast<ID3D11DepthStencilView*>(const_cast<void*>(GetGraphID3D11DepthStencilView(dxlib_handle)));
	if (desc.BindFlags & D3D11_BIND_SHADER_RESOURCE)
	{
		if (desc.Format == DXGI_FORMAT_R32_TYPELESS) {    // デプスバッファ用の場合はR32_FLOATとして利用
			D3D11_SHADER_RESOURCE_VIEW_DESC view_desc{};
