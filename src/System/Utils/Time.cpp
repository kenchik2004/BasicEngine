//---------------------------------------------------------------------------
//! @file   Time.cpp
//! @brief  時間管理ユーティリティの実装。FPS制御・タイムスケール・Δ時間の計算を行う
//---------------------------------------------------------------------------
#include "Time.h"
...
namespace Time {
	...
	int FixFPS()
	{
		...
		std::this_thread::sleep_for(std::chrono::milliseconds(sleep_time_int));					//!<sleep時間だけアプリケーションを停止
	...
	void FixFixedFPS(u64 loops) {
		fixed_fps = 1.0 / fixed_delta_time;						//!<物理FPSを計測
		fixed_delta_time -= fixed_delta_time_max * loops;				//!<そして1フレーム分デクリメント(こちらは0にすると物理演算が不正確になるのでデクリメントして使う)
